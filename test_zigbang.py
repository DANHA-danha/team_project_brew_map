...
# overwrite=True로 매 실행마다 파일 덮어쓰기
    ...
    def parse_stores(self, response):
        self.logger.info("stores_status=%s", response.status)
        self.logger.info("stores_body_head=%s", response.text[:500])
        data = json.loads(response.text)
        self.logger.info("stores_json_type=%s", type(data).__name__)
